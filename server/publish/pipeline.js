...
async function fetchRuntimeHealth(runtime, options = {}) {
  ...
  const mode = String(process.env.RUNTIME_ACCESS_MODE || 'container').toLowerCase();
  let runtimeBaseUrl = '';

  if (mode !== 'host') {
    // 容器模式：优先走容器 IP:3001，其次才回退到宿主 loopback 端口
    if (runtime.ip) runtimeBaseUrl = `http://${runtime.ip}:3001`;
    else if (runtime.hostPort) runtimeBaseUrl = `http://127.0.0.1:${runtime.hostPort}`;
  } else {
    // 宿主模式：优先通过 127.0.0.1:hostPort 访问
    if (runtime.hostPort) runtimeBaseUrl = `http://127.0.0.1:${runtime.hostPort}`;
    else if (runtime.ip) runtimeBaseUrl = `http://${runtime.ip}:3001`;
  }
