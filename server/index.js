...
function getRuntimeBaseUrl(runtime) {
  ...
  // 容器模式（平台跑在 Docker 内）：优先使用容器 IP:3001，其次才回退到 hostPort（宿主调试用）
  if (mode !== 'host') {
  ...
  // 宿主机模式（平台直接跑在 host 上）：优先通过 127.0.0.1:hostPort 访问
  if (runtime.hostPort) return `http://127.0.0.1:${runtime.hostPort}`;
  if (runtime.ip) return `http://${runtime.ip}:3001`;
  return '';
}
...
function proxyToAppContainer(req, res, runtime, pathPart) {
  ...
  if (mode !== 'host') {
    ...
    if (runtime.ip) {
      targetHost = runtime.ip;
      targetPort = 3001;
    } else if (runtime.hostPort) {
      targetHost = '127.0.0.1';
      targetPort = runtime.hostPort;
    }
  } else {
