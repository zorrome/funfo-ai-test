...
function App() {
  ...
  function handleRegister() {
    ...
    var newUser = { id: Date.now(), name: name.trim(), gender: gender, age: ageNum, created_at: new Date().toISOString() };
  ...
  function saveEdit() {
    ...
    setUsers(function(prev) {
      return prev.map(function(u) {
        if (u.id === editId) return Object.assign({}, u, { name: editName.trim(), gender: editGender, age: ageNum });
        return u;
      });
    });
    setEditId(null);
  ...
  var stats = useMemo(function() {
    ...
    if (total === 0) return { total: 0, male: 0, female: 0, other: 0, avgAge: 0, minAge: 0, maxAge: 0, ageGroups: [] };
    var male = safeUsers.filter(function(u) { return u.gender === "男性"; }).length;
    var female = safeUsers.filter(function(u) { return u.gender === "女性"; }).length;
    var other = total - male - female;
    var ages = safeUsers.map(function(u) { return u.age; });
    var sum = ages.reduce(function(a, b) { return a + b; }, 0);
    var groups = [
      { name: "0-17歳", min: 0, max: 17, count: 0, color: "#60a5fa" },
      { name: "18-29歳", min: 18, max: 29, count: 0, color: "#34d399" },
      { name: "30-49歳", min: 30, max: 49, count: 0, color: "#fbbf24" },
      { name: "50-69歳", min: 50, max: 69, count: 0, color: "#f87171" },
      { name: "70歳+", min: 70, max: 999, count: 0, color: "#a78bfa" }
    ];
    safeUsers.forEach(function(u) {
      groups.forEach(function(g) { if (u.age >= g.min && u.age <= g.max) g.count++; });
    });
    return { total: total, male: male, female: female, other: other, avgAge: Math.round(sum / total * 10) / 10, minAge: Math.min.apply(null, ages), maxAge: Math.max.apply(null, ages), ageGroups: groups };
  }, [safeUsers]);

  var genderData = [
    { name: "男性", value: stats.male, color: "#3b82f6" },
    { name: "女性", value: stats.female, color: "#ec4899" },
    { name: "その他", value: stats.other, color: "#8b5cf6" }
  ].filter(function(d) { return d.value > 0; });

  var ageChartData = stats.ageGroups.filter(function(g) { return g.count > 0; });
        ...
        {view === "stats" && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              {[
                { label: "総ユーザー数", value: stats.total, unit: "人", color: "from-indigo-500 to-blue-500", emoji: "👥" },
                { label: "平均年齢", value: stats.avgAge, unit: "歳", color: "from-emerald-500 to-teal-500", emoji: "📅" },
                { label: "最年少", value: stats.total > 0 ? stats.minAge : "-", unit: stats.total > 0 ? "歳" : "", color: "from-amber-500 to-orange-500", emoji: "🌱" },
                { label: "最年長", value: stats.total > 0 ? stats.maxAge : "-", unit: stats.total > 0 ? "歳" : "", color: "from-rose-500 to-pink-500", emoji: "🌳" }
              ].map(function(card) {
                return (
